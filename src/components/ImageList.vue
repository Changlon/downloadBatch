...
export default { 
    ...
    setup() { 
        ...
        let queryDetial = media_key =>{    
            $router.push({params:{...postParams.value,mediaKey:media_key},name:"ImageDownload",query:{timestamp : new Date().getTime()}})
        } 
        ...
        let downloadBatch = async ()=>{
            const openid = postParams.value.openid 
            const mediaList = [] 
           ...
           if(openid && mediaList.length > 0 ) {  
                ...
                Notify({ type: 'danger', message: "用户数据刷新丢失,重新从公众号打开！",duration:1000})
           }
